...
#     # sum in log space is not correct
#     return jnp.log(0.25 * (jnp.exp(g1) + jnp.exp(g2) + jnp.exp(g3) + jnp.exp(g4))).sum(
#         axis=-1
#     )
def target_logdensity_fn(x: Array) -> Float:
    # mixture of four Gaussian in 2D
    assert x.shape[-1] == 2  # explicitly check for 2D
    
    # Define the centers
    mu1 = jnp.array([2.0, 2.0])
    mu2 = jnp.array([-2.0, -2.0])
    mu3 = jnp.array([2.0, -2.0])
    mu4 = jnp.array([-2.0, 2.0])
    
    # Calculate log-densities for each component
    # For a multivariate normal with diagonal covariance
    sigma = 0.5
    g1 = jsp.stats.multivariate_normal.logpdf(x, mu1, jnp.eye(2) * sigma**2)
    g2 = jsp.stats.multivariate_normal.logpdf(x, mu2, jnp.eye(2) * sigma**2)
    g3 = jsp.stats.multivariate_normal.logpdf(x, mu3, jnp.eye(2) * sigma**2)
    g4 = jsp.stats.multivariate_normal.logpdf(x, mu4, jnp.eye(2) * sigma**2)
    
    # Log of weighted sum of component densities
    # log(0.25 * (exp(g1) + exp(g2) + exp(g3) + exp(g4)))
    log_weights = jnp.log(jnp.array([0.25, 0.25, 0.25, 0.25]))
    return jsp.special.logsumexp(jnp.array([g1, g2, g3, g4]) + log_weights)
...
approx_samples_cpu = jax.device_get(approx_samples)
sns.scatterplot(x=approx_samples_cpu[:, 0], y=approx_samples_cpu[:, 1], alpha=0.5)
plt.title('Approximated Samples')
plt.savefig('approximated_samples.png')
